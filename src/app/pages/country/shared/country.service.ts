...
import { CountryRelationCreate, CountryResponse, CountrySelect, CountryUpdateRequest } from './country.model';
...
export class CountryService {
  ...
  updateCountry(request: CountryUpdateRequest) {
    return this.api.put(CountryApi.UPDATE_COUNTRY.replace("#", request.id.toString()), request).pipe(
      map(response => {
        return response;
      })
    );
