...
export class GalleryCreateComponent implements OnInit
{
    ...
    items: [
      {
        ...
        action: () =>
        {
          this.router.navigate(['./gallery/' + this.objectType + '/' + this.objectId])
        },
      },
    ],
  });
    ...
    items: [
      {
        ...
        {
          if (
            this.model.image_files &&
            this.model.image_files[0]
          )
          {
            this.readFile();
          }
  ...
  saveImage = async () =>
  {
    if (this.imageFormConfig.isValid) 
    {
      ...
      let mediaCreateRequest = new MediaCreateRequest(this.objectType, this.objectId,
        this.model.imageData as string, this.model.image, MediaConstant.MEDIA_OBJECT_TYPE_IMAGE,
        MediaConstant.MEDIA_STORE_TYPE_COVER_IMAGE, '');


      this.galleryService.addNewImage(mediaCreateRequest).subscribe(response =>
      {
        if (response)
        {
